...
const sendSMS = async (mobile, otp) => {
  try {
    ...
    if (!twilioClient) {
      console.log(
        `📱 [DEV MODE - SMS] ${normalizedMobile}: Your MedLink OTP is ${otp}`
      );
      return {
        success: true,
...
const sendWhatsApp = async (mobile, otp) => {
  try {
    ...
    if (!twilioClient || !process.env.TWILIO_WHATSAPP_NUMBER) {
      console.log(
        `📱 [DEV MODE - WhatsApp] ${normalizedMobile}: Your MedLink OTP is ${otp}`
      );
      return {
        success: true,
        message: "OTP logged (development mode)",
    ...
    console.log(
      `✅ WhatsApp Sent: SID=${message.sid} Status=${message.status} To=${to}`
    );
    return { success: true, messageId: message.sid };
  } catch (error) {
...
const sendOTP = async ({
  ...
  try {
    ...
    if (queryConditions.length === 0) {
      throw new Error(
        "At least one identifier (email/mobile) must be provided."
      );
    }
    ...
    const results = await Promise.allSettled([
      mobile
        ? sendSMS(mobile, otp)
        : Promise.resolve({ success: false, error: "No mobile provided" }),
      mobile
    ...
    results.forEach((result, index) => {
      ...
      if (
        isFulfilled &&
        result.value.error !== "No mobile provided" &&
        result.value.error !== "No email provided"
      ) {
        notifications.push({
          recipient: {
...
const verifyOTP = async ({ email, mobile, otp, purpose }) => {
  try {
    ...
    if (otpLog.attempts >= otpLog.maxAttempts) {
      return {
        success: false,
        message: "Max attempts reached. Request a new OTP.",
      };
    }
    ...
    if (!isMatch) {
      ...
      return {
        success: false,
        message: `Invalid OTP. ${
          otpLog.maxAttempts - otpLog.attempts
        } attempts left.`,
      };
    }
